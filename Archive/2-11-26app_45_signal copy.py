...
r2_window = st.sidebar.selectbox(
    "R² Window (days)",
    [30, 60, 90, 120],
    index=1,  # default: 60
    help="Window for R² consistency score (Component B). Shorter = recent trend, Longer = established trend."
)
    ...
    if st.button("🚀 Run Scan", type="primary", disabled=preview_count == 0):
        ...
        if len(ticker_list) == 0:
            ...
            if df_results.empty:
                ...
                wanted = ["Ticker", "Score", "SIGNAL-Super-MOST-ADXR", "Earnings_Alert",
                          "Market_Cap", "Price", "RS_63d_vs_SPY_%", "Dist_to_21EMA_%"]
                st.dataframe(style_dataframe(df_results[safe_cols(df_results, wanted)].head(10)), width="stretch")
