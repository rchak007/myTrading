...
def calculate_45_degree_score(
    ...
    """
    Calculate 45° uptrend score for a single R² window.

    Parameters:
    -----------
    r2_window : int
        Window used for Component A (slope) and Component B (R² consistency).
        Component D always uses 60-day momentum as secondary confirmation
        (only meaningful when r2_window != 60; otherwise skipped to avoid redundancy).
    """
    ...
    # Regression — main window (configurable) + 60-day momentum
    slope_main, r2_main = regression_stats(c, r2_window, log=True)
    ...
    # (A) Slope — 45° angle check using main window
    if pd.notna(slope_main):
        radians   = np.arctan(slope_main)
        angle_deg = np.degrees(radians)
        diff = abs(slope_main - 0.00274)
        if diff < 0.00137:
            score += 30
        elif diff < 0.00274:
            score += 15

    # (B) R² Consistency — main window
    if pd.notna(r2_main):
        if r2_main >= 0.85:
            score += 20
        elif r2_main >= 0.70:
    ...
    # (D) Short-term momentum — only add when main window isn't already 60d
    # (avoids double-counting the same calculation)
    if r2_window != 60:
        if pd.notna(slope60) and slope60 > 0:
            score += 5
    ...
    return {
        "score":          int(score),
        "slope_main":     float(slope_main) if pd.notna(slope_main) else np.nan,
        "r2_main":        float(r2_main)    if pd.notna(r2_main)    else np.nan,
...
def calculate_all_scores(
    ...
    """
    Calculate scores for all four R² windows (30, 60, 90, 120 days).
    Returns a flat dict with Score_30, Score_60, Score_90, Score_120,
    plus Score_Weighted (recency-weighted average),
    plus the full detail from the 60-day window (used as primary).
    """
    scores = {}
    detail = {}
    for w in [30, 60, 90, 120]:
        result = calculate_45_degree_score(df, spy_close, r2_window=w)
        scores[f"Score_{w}"] = result["score"]
        if w == 60:
            detail = result   # keep 60d detail as primary for other fields

    # Calculate weighted score (recency bias: 50% 30d, 30% 60d, 15% 90d, 5% 120d)
    score_weighted = (
        scores["Score_30"]  * 0.50 +
        scores["Score_60"]  * 0.30 +
        scores["Score_90"]  * 0.15 +
        scores["Score_120"] * 0.05
    )
